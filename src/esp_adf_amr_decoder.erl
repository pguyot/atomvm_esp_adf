...
-module(esp_adf_amr_decoder).

-export([init/1]).

-type amr_decoder_option() ::
...
%% @doc Create a handle to an AMR decoder audio element.
%% @param Cfg configuration
%% @return an audio element resource
-spec init([amr_decoder_option()]) -> esp_adf_audio_element:audio_element().
init(_Cfg) ->
