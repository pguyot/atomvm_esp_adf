...
-module(esp_adf_wav_decoder).

-export([init/1]).

-type wav_decoder_option() ::
...
%% @doc Create a handle to an WAV decoder audio element.
%% @param Cfg configuration
%% @return an audio element resource
-spec init([wav_decoder_option()]) -> esp_adf_audio_element:audio_element().
init(_Cfg) ->
