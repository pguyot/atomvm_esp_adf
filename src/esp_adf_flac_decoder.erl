...
-module(esp_adf_flac_decoder).

-export([init/1]).

-type flac_decoder_option() ::
...
%% @doc Create a handle to an FLAC decoder audio element.
%% @param Cfg configuration
%% @return an audio element resource
-spec init([flac_decoder_option()]) -> esp_adf_audio_element:audio_element().
init(_Cfg) ->
