...
#ifdef CONFIG_AVM_ESP_ADF_PCM_DECODER_ENABLE
...
#define MODULE_PREFIX "esp_adf_pcm_decoder:"
#define TAG "esp_adf_pcm_decoder"
...
static term nif_init(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xB", "out_rb_size"), &pcm_cfg.out_rb_size, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xA", "task_stack"), &pcm_cfg.task_stack, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_core"), &pcm_cfg.task_core, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_prio"), &pcm_cfg.task_prio, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_bool_parameter(ctx, argv, ATOM_STR("\xC", "stack_in_ext"), &pcm_cfg.stack_in_ext))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x4", "rate"), &pcm_cfg.rate, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x4", "bits"), &pcm_cfg.bits, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x8", "channels"), &pcm_cfg.channels, false))) {
        return term_invalid_term();
    }
...
REGISTER_NIF_COLLECTION(esp_adf_pcm_decoder, NULL, NULL, get_nif)
