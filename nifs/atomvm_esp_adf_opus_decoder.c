...
#ifdef CONFIG_AVM_ESP_ADF_OPUS_DECODER_ENABLE
...
#define MODULE_PREFIX "esp_adf_opus_decoder:"
#define TAG "esp_adf_opus_decoder"
...
static term nif_init(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xB", "out_rb_size"), &opus_cfg.out_rb_size, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xA", "task_stack"), &opus_cfg.task_stack, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_core"), &opus_cfg.task_core, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_prio"), &opus_cfg.task_prio, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_bool_parameter(ctx, argv, ATOM_STR("\xC", "stack_in_ext"), &opus_cfg.stack_in_ext))) {
        return term_invalid_term();
    }
    audio_element_handle_t audio_element = decoder_opus_init(&opus_cfg);
...
REGISTER_NIF_COLLECTION(esp_adf_opus_decoder, NULL, NULL, get_nif)
