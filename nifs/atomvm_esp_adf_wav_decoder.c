...
#ifdef CONFIG_AVM_ESP_ADF_WAV_DECODER_ENABLE
...
#define MODULE_PREFIX "esp_adf_wav_decoder:"
#define TAG "esp_adf_wav_decoder"
...
static term nif_init(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xB", "out_rb_size"), &wav_cfg.out_rb_size, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xA", "task_stack"), &wav_cfg.task_stack, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_core"), &wav_cfg.task_core, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_prio"), &wav_cfg.task_prio, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_bool_parameter(ctx, argv, ATOM_STR("\xC", "stack_in_ext"), &wav_cfg.stack_in_ext))) {
        return term_invalid_term();
    }
...
REGISTER_NIF_COLLECTION(esp_adf_wav_decoder, NULL, NULL, get_nif)
