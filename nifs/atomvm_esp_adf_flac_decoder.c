...
#ifdef CONFIG_AVM_ESP_ADF_FLAC_DECODER_ENABLE
...
#define MODULE_PREFIX "esp_adf_flac_decoder:"
...
static term nif_init(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xB", "out_rb_size"), &flac_cfg.out_rb_size, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\xA", "task_stack"), &flac_cfg.task_stack, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_core"), &flac_cfg.task_core, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_integer_parameter(ctx, argv, ATOM_STR("\x9", "task_prio"), &flac_cfg.task_prio, false))) {
        return term_invalid_term();
    }
    if (UNLIKELY(!get_bool_parameter(ctx, argv, ATOM_STR("\xC", "stack_in_ext"), &flac_cfg.stack_in_ext))) {
        return term_invalid_term();
    }
    audio_element_handle_t audio_element = flac_decoder_init(&flac_cfg);
...
REGISTER_NIF_COLLECTION(esp_adf_flac_decoder, NULL, NULL, get_nif)
